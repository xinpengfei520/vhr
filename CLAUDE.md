...
`vhr-web` will not start without all of these reachable at the addresses in `vhr/vhrserver/vhr-web/src/main/resources/application.yml` (note: `vhr-web` uses **YAML**; `mailserver` still uses `application.properties`):
...
- **RabbitMQ** on `localhost:5672` — the exchange/queue/routing-key names are constants in `vhr-model` (`MailConstants`) and must be identically configured in both `vhr-web`'s `application.yml` and `mailserver`'s `application.properties`.
- **FastDFS + Nginx** — only required if exercising avatar / file-upload flows; the Nginx endpoint is `fastdfs.nginx.host` in `application.yml`, and the tracker/server list lives in the sibling `fastdfs-client.properties`.
...
- **Auth / authorization is dynamic**: `SecurityConfig` (in `vhr-web/config/`) wires `CustomFilterInvocationSecurityMetadataSource` (loads menu→role rules from DB) and `CustomUrlDecisionManager` (evaluates them per request). A custom `LoginFilter` (extends `UsernamePasswordAuthenticationFilter`) parses JSON login bodies **and** performs captcha check against `verify_code` in session (source: `VerificationCode`) — do not add a separate captcha filter, that path was consolidated during the multi-device kick-out refactor. Concurrent-session control uses Spring Security's `SessionRegistry` + `ConcurrentSessionFilter`, so a second login for the same user kicks the first out. Role/menu data drives the frontend menu tree too — the same `MenuService` endpoint feeds both authorization and UI.
...
Committed local credentials live in `vhr/vhrserver/vhr-web/src/main/resources/application.yml` (DB / Redis / RabbitMQ / FastDFS Nginx) and `vhr/mailserver/src/main/resources/application.properties` (SMTP + RabbitMQ + Redis). Treat them as developer defaults, not secrets to reuse — the RabbitMQ / Redis passwords in particular differ between the two files by upstream default and must be aligned locally, since the mail flow silently drops messages when the two configs disagree.
